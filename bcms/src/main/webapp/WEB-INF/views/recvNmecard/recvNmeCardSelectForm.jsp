...
<html>
	...
	//받은 명함 정렬보기
		function arrayNmeCard(page) {
		 ...
		 function deleteRecvNmeCard(){ //명함 체크박스 삭제
				if (confirm("삭제하시겠습니까?") == true){    //확인
					var nmeCardSeq ="";
					var checkArray = new Array();  
					<c:forEach var="selectRecvNmeCardList" items="${selectRecvNmeCardList}" varStatus="status">
						nmeCardSeq= "#"+"${selectRecvNmeCardList.nmeCardSeq}";
						if($(nmeCardSeq).is(":checked")){
							checkArray.push("${selectRecvNmeCardList.nmeCardSeq}");			
						}
					</c:forEach>
					$.ajax({
					type : 'POST',
					url : 'deleteRecvNmeCard.do',
					data : {  
						0:0,
	...
	<body>
		...
		<form name="nmeCardInfo" method="post">		
			...
			<input type="hidden" name="comnctnSharSeq" value="0">
			<input type="hidden" name="recvNmeCardBinUseAt" value="${nmeCardValueObject.recvNmeCardBinUseAt}">
		...
		<div class="body" style="margin-bottom:3%">
			<div class ="wrapper">
				...
				<div class="card">
					...
					<button class="btn btn-default-mina" style="display:inline; margin-bottom:5px; margin-left:7px" onclick="location.href='/recvNmeCardSelectForm.do'"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span></button>				
					<div class="btn-group" style="margin-bottom:5px">
						...
						<ul class="dropdown-menu" role="menu">
							...
							<li><a href="#" onclick="deleteRecvNmeCard()" id="del">삭제</a></li>
						</ul>
					</div>
					</h3>
					<article>
								...
								<ul id="productList">
									<c:forEach var="selectRecvNmeCardList" items="${selectRecvNmeCardList}" varStatus="status">
										<li id="selectCardList" style="margin-left:20px;">
											...
											<div id="cardBox" class="cardBox${selectRecvNmeCardList.nmeCardSeq}" style="float:left;">
												<a href="javascript:recvNmeCardSelectViewForm(${selectRecvNmeCardList.nmeCardSeq})" id="cardId" onmouseover="this.style.color='#428BCA'" onmouseout="this.style.color='#333333'" style="text-decoration:none">
													<table class="tb-radius ellip" id="nmecardList">
														<tr>
															<td>
															성명 : <c:out value="${selectRecvNmeCardList.nmeCardNme}"/>
															<c:if test="${selectRecvNmeCardList.recvNmeCardDateUseAt == 'Y'}">
														...
														<tr>
															<c:if test="${selectRecvNmeCardList.nmeCardEmail == ''}">
															<td>E-mail : 없음</td>
															</c:if>
															<c:if test="${selectRecvNmeCardList.nmeCardEmail != ''}">
														...
														<tr>
															<td>회사 : <c:out value="${selectRecvNmeCardList.nmeCardCompnNme}"/></td>
														</tr>
													</table>
												</a>
											</div>
