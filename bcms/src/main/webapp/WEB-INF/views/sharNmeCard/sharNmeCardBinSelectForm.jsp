...
<html>
	<head>
		<meta charset="UTF-8">
		<title>BCMS - 공유 명함 리스트 휴지통</title>
		...
		<script type="text/javascript">
			...
			//상세명함
			function sharNmeCardViewForm(sharSeq, nmeCardSeq, sharUseAtBinType, sharType) { 
				...
				document.nmeCardInfo.sharSeq.value = sharSeq;
				document.nmeCardInfo.nmeCardSeq.value = nmeCardSeq;
				document.nmeCardInfo.sharUseAtBinType.value = sharUseAtBinType;
				document.nmeCardInfo.sharType.value = sharType;
				...
				document.nmeCardInfo.action = "/sharNmeCardViewForm.do";
			...
			function arrayNmeCard(page) { //명함 정렬보기
				...
				document.nmeCardInfo.searchWrd.value = searchWrd;
				document.nmeCardInfo.searchAry.value = searchAry;
				document.nmeCardInfo.action = "/sharNmeCardBinSelectForm.do";
				document.nmeCardInfo.submit();
			}
...
// 			}
			...
			function deleteNmeCardBin(){ //명함 체크박스 삭제
				...
				<c:forEach var="selectSharNmeCardList" items="${selectSharNmeCardList}" varStatus="status">
					nmeCardSeq= "#"+"${selectSharNmeCardList.sharSeq}";
					if($(nmeCardSeq).is(":checked")){
						checkArray.push("${selectSharNmeCardList.sharSeq}");			
					}
				</c:forEach>
			...
			function recvySharNmeCardBin() { //명함 체크박스 복원
				var nmeCardSeq ="";
				var checkArray = new Array();  
				<c:forEach var="selectSharNmeCardList" items="${selectSharNmeCardList}" varStatus="status">
					nmeCardSeq= "#"+"${selectSharNmeCardList.sharSeq}";
	...
	<body>
		...
		<form name="nmeCardInfo" method="post">		
			<input type="hidden" name="sharSeq" value="${nmeCardValueObject.sharSeq}">
			<input type="hidden" name="nmeCardSeq" value="${nmeCardValueObject.nmeCardSeq}">
			<input type="hidden" name="sharUseAtBinType" value="${nmeCardValueObject.sharUseAtBinType}">
			<input type="hidden" name="sharType" value="${nmeCardValueObject.sharType}">
		...
		<div class="body" style="margin-bottom:3%">
			<div class ="wrapper">
				...
				<div class="card">
					...
					<button class="btn btn-default" style="margin-left:7px;" onclick="location.href='/sharNmeCardBinSelectForm.do'"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span></button>				
					<div class="btn-group">
						...
						<ul class="dropdown-menu" role="menu">
							...
							<li><a href="#" onclick="javascript:recvySharNmeCardBin()">복원</a></li>
									...
									<c:forEach var="selectSharNmeCardList" items="${selectSharNmeCardList}" varStatus="status">
										<li id="selectCardList" style="margin-left:20px;">
											<div style="float:left">
												<input type="checkbox" id="${selectSharNmeCardList.sharSeq}" name="chkbox" value="${selectSharNmeCardList.sharSeq}">
											</div>
											<div id="cardBox" class="cardBox${selectSharNmeCardList.nmeCardSeq}" style="float:left;">
												<a href="javascript:sharNmeCardViewForm(${selectSharNmeCardList.sharSeq}, ${selectSharNmeCardList.nmeCardSeq}, '${selectSharNmeCardList.sharUseAtBinType}', '${selectSharNmeCardList.sharType}')" onmouseover="this.style.color='#428BCA'" onmouseout="this.style.color='#333333'" style="text-decoration:none">
													<table class="tb-radius" id="nmecardList">
														<tr>											
															<td><input type="hidden" value="${selectSharNmeCardList.sharSeq}">
															성명 : <c:out value="${selectSharNmeCardList.nmeCardNme}"/></td>
														</tr>
														<tr>
															<td>휴대폰 : <c:out value="${selectSharNmeCardList.nmeCardMobil}"/></td>
														</tr>
														<tr>
															<td>E-mail : <c:out value="${selectSharNmeCardList.nmeCardEmail}"/></td>
														</tr>
														<tr>
															<td>전화 : <c:out value="${selectSharNmeCardList.nmeCardTel}"/></td>
														</tr>
														<tr>
															<td>회사 : <c:out value="${selectSharNmeCardList.nmeCardCompnNme}"/></td>
														</tr>
													</table>
												</a>
											</div>
