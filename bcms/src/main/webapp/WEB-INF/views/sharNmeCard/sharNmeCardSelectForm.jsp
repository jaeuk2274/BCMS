	...
	<head>
		...
		<script type="text/javascript">
			...
			function arrayNmeCard(page) { 
				...
				document.nmeCardInfo.searchWrd.value = searchWrd;
				document.nmeCardInfo.searchAry.value = searchAry;
				document.nmeCardInfo.action = "/sharNmeCardSelectForm.do";
				document.nmeCardInfo.submit();
			}
			...
			function deleteNmeCard(){ //명함 체크박스 삭제
				...
				<c:forEach var="selectNmeCardList" items="${selectNmeCardList}" varStatus="status">
					nmeCardSeq= "#"+"${selectNmeCardList.sharSeq}";
					if($(nmeCardSeq).is(":checked")){
						checkArray.push("${selectNmeCardList.sharSeq}");			
					}
				</c:forEach>
		...
		<div class="body" style="margin-bottom:60px">
			<div class ="wrapper">
				...
				<div class="card">
					...
					<button class="btn btn-default" style="margin-left:7px;" style="display:inline" onclick="location.href='/sharNmeCardSelectForm.do'"><span class="glyphicon glyphicon-repeat" aria-hidden="true"></span></button>				
					<div class="btn-group">
									...
									<c:forEach var="selectNmeCardList" items="${selectNmeCardList}" varStatus="status">
										<li id="selectCardList" style="margin-left:20px;">
											<div style="float:left">
												<input type="checkbox" id="${selectNmeCardList.sharSeq}" name="chkbox" value="${selectNmeCardList.nmeCardSeq}">
											</div>
											<div id="cardBox" class="cardBox${selectNmeCardList.nmeCardSeq}" style="float:left;">
												<a href="javascript:sharNmeCardViewForm(${selectNmeCardList.sharSeq}, ${selectNmeCardList.nmeCardSeq}, '${selectNmeCardList.sharUseAtBinType}', '${selectNmeCardList.sharType}')" onmouseover="this.style.color='#428BCA'" onmouseout="this.style.color='#333333'" style="text-decoration:none">
													<table class="tb-radius" id="nmecardList">
														<tr>
															<td>
															<input type="hidden" value="${selectNmeCardList.sharSeq}">
															성명 : <c:out value="${selectNmeCardList.nmeCardNme}"/>
															<c:if test="${selectNmeCardList.sharCretDate == 'Y'}">
