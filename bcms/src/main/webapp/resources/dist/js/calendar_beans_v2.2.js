...
function CalAddCss(){
		...
		cssHtml += ".dayToday{background-color: #368CFE;border:1px solid #CBC7BD;font-weight:bold;font-weight:bold;width:22px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;color:white;	}";
		cssHtml += ".calSelDay{	background-color: #CF4040;border:1px solid #CBC7BD;font-weight:bold;width:22px;float:left;font-size:8pt;padding-top:3px;padding-bottom:2px;cursor:pointer;text-decoration:underline;color:white;}";
...
function initCal(obj){
	...
	var maxYear 	= obj.max == null ? new Date().getFullYear()+50 : obj.max;
	...
	if( calType == "day"){
		...
		$("#"+calId +"btn.todayBtn").css("width","167px");							
	}else{
		$("#"+obj.id).attr("maxlength",7);
		$("#"+calId).css("width","156px");
		$("#"+calId +"btn.todayBtn").css("width","143px");
	}
